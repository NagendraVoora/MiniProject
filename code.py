...
yawn_times=[]
# To track the timestamp of yawnings
sleep_times=[]
...
sleep_end=time.time() # to capture sleep instace end
drowsy = 0
hours=0
minutes=0
seconds=0
...
def is_mouth_opened(landmarks):
    ...
    lip_upper_mean = np.mean(top_lip, axis=0)
    lip_lower = landmarks[56:59]
    lip_lower_mean = np.mean(bottom_lip, axis=0)
    distance = np.abs(lip_upper_mean[1] - lip_lower_mean[1])
    return distance > 25

def play_alert_sound():
    # Replace "alert.wav" with the path to your sound file
    # playsound("alert.wav")
    load_file = sa.WaveObject.from_wave_file("alert.wav")
    play = load_file.play()
    play.wait_done()
    
def CALC_HOURLY_CLASSIFICATION(data,start,end):
    hours=math.ceil((end-start)/3600)
    Lists=[]
    for i in range(hours):
        Min=0
        Max=0
        for j in range(len(data)):
            if data[j]>=(start+3600*i):
                Min=j
                break
        for k in range(Min,len(data)):
            if data[k]<(start+3600*(i+1)):
                Max=k
        if Max>=Min:
...
while True:
    ...
    for f in faces:
        ...
        landmarks_in_face = face_predict(gray, face)
        landmarks_in_face = face_utils.shape_to_np(landmarks_in_face)

        left_blink = check_blinking(landmarks_in_face[36], landmarks_in_face[37], landmarks_in_face[38], landmarks_in_face[41], landmarks_in_face[40], landmarks_in_face[39])
        right_blink = check_blinking(landmarks_in_face[42], landmarks_in_face[43], landmarks_in_face[44], landmarks_in_face[47], landmarks_in_face[46], landmarks_in_face[45])
...
if no_of_sleeps>0 and no_of_yawns>(3*Hours_of_driving) and no_of_blinks>(5*Hours_of_driving):
    file.write("Drivers didn't driving carefully, It look like he didnt had proper sleep, it may dangerous for passangers.\n He is not good fit for driving")
elif no_of_sleeps>0 and no_of_yawns>(3*Hours_of_driving):
    file.write("Drivers feeling drowsy more than usual.\n Can be eligible for driving short distances with some more attention")
    ...
    file.write("Suitable for short drives and can be sent to long drives if some more attention posessed")
else:
    file.write("Driver is abosolutely fine and He is good fit for long drives")
